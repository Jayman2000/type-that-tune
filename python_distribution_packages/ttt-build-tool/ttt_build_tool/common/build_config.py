...
Some of the functions in this module do something like this in order to
copy their parameters before using them:
...
from . import EXPORTED_DIR, download, run_command
...
class GodotEditorSearchTuple(SearchTuple):
    ...
    def downloaded_godot_editor_path() -> pathlib.Path:
        ...
        """
        ZIP_FILE_PATH: Final = download.download_if_needed(
        ...
        for item in self:
            ...
            if path_to_test is not None:
                try:
                    ...
                    run_command.NonZeroReturnCodeError,
                    FileNotFoundError
                ):
                    print("Test failed.")
            warnings.warn(
